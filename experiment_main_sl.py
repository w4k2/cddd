...
def compute(clf_name, clf, drift, n_drifts, random_state, experiment_name, concept_kwargs):
    ...
    try:
        ...
        if drift == 'incremental':
            ...
            concept_kwargs['recurring'] = True

        stream_size = (concept_kwargs["n_chunks"]*concept_kwargs["chunk_size"]) / 1000
        stream_name = "stream_sl_%dd_%s_%03dk_f%02d_rs%03d" % (n_drifts, drift[0], stream_size, concept_kwargs["n_features"], random_state)
...
drifts = ['sudden', 'incremental', 'recurring', 'gradual']
