...
import numpy as np
from river_wrapper import RiverRTStream
...
def compute(clf_name, clf, drift, n_drifts, random_state, experiment_name, concept_kwargs):
    ...
    try:
        ...
        stream_name = "stream_rv_%dd_%s_%03dk_f%02d_rs%03d" % (n_drifts, drift[0], stream_size, concept_kwargs["n_features"], random_state)

        if os.path.exists("results/raw_conf/%s/rv_%dd/%s/%s/%s.csv" % (experiment_name, n_drifts, drift, stream_name, clf_name)):
            return
        ...
        stream = RiverRTStream(n_chunks=concept_kwargs["n_chunks"],
        ...
        filename = "results/raw_conf/%s/rv_%dd/%s/%s/%s.csv" % (experiment_name, n_drifts, drift, stream_name, clf_name)
        if not os.path.exists("results/raw_conf/%s/rv_%dd/%s/%s/" % (experiment_name, n_drifts, drift, stream_name)):
            os.makedirs("results/raw_conf/%s/rv_%dd/%s/%s/" % (experiment_name, n_drifts, drift, stream_name))
